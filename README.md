...
Released approvals offer **Accept invitation** and **Decline invitation** on the dashboard, even when registration has closed or the form is inactive. The final answer is stored in `invitation_response` (`accepted`, `declined`, or null) with a database-generated `invitation_responded_at`. Identical retries preserve the original answer and timestamp; an opposite answer is rejected. There is no automatic expiry. Releasing a waitlist/rejection stops new responses, but retains any previous answer. Reapproval also retains that answer; organizers can correct an accepted or declined response from the applicant’s **Review → Change invitation response** action while their released decision is approved. The confirmation updates the response and its timestamp immediately, without sending email or changing either decision. A concurrent response change rejects the correction; reopen Review before retrying. This uses the existing columns and requires no additional migration.
