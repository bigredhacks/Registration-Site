...
export default function Toast({ message, type, onDismiss }: ToastProps) {
  return (
    <div
      ...
      className={`pointer-events-auto flex w-full max-w-sm items-center gap-3 rounded-lg px-5 py-3.5 shadow-xl font-poppins text-sm toast-enter ${styles[type]}`}
    >
      <span className="text-base font-bold shrink-0">{icons[type]}</span>
      <span className="min-w-0 flex-1 [overflow-wrap:anywhere] leading-snug">{message}</span>
      <button
        type="button"
        onClick={onDismiss}
