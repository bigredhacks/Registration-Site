...
import { applicationAnswers, formatUserAnswer, profileStateLabel, userName, type AdminUserDetail as UserDetail } from './adminUsersState';
...
export default function AdminUserDetail({ detail, formTitle }: { detail: UserDetail; formTitle: string }) {
  ...
  return <>
    ...
    <section aria-label="User profile" className="mt-6">
      ...
      <p className="admin-meta mb-3">{profileStateLabel(detail.profile_state)} · {detail.profile_pct}% complete</p>
    ...
    <section aria-label="User application" className="mt-6">
      ...
      {!application ? <p className="admin-meta">No application submitted for this form.</p> : <>
        ...
        <p className="admin-meta mt-2">Saved decision: {decisionOptions.find(option => option.value === application.status)?.label ?? application.status}</p>
